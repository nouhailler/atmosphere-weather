...
function TempChart({ hourly, unit }) {
  ...
  const chartData = {
    labels,
    datasets: [{ data:temps, borderColor:"#5bb1ff", backgroundColor:"rgba(91,177,255,0.12)", borderWidth:2, pointRadius:0, pointHoverRadius:5, pointHoverBackgroundColor:"#5bb1ff", fill:true, tension:0.4 }],
  };
  const options = {
    responsive:true, maintainAspectRatio:false,
    animation:{ duration:700, easing:"easeOutQuart" },
    plugins:{ legend:{ display:false }, tooltip:{ backgroundColor:"rgba(12,25,52,0.95)", titleColor:"#dee5ff", bodyColor:"#9baad6", borderColor:"rgba(91,177,255,0.3)", borderWidth:1, padding:10, callbacks:{ label:ctx=>` ${ctx.raw}${sym}` } } },
    scales:{
    ...
    <section style={s.hero}>
      ...
      {displayUrl && (
        <div style={{ position:"absolute", top:20, right:20, display:"flex", alignItems:"center", gap:8, backgroundColor:"rgba(6,14,32,0.65)", backdropFilter:"blur(8px)", padding:"5px 10px", borderRadius:9999, border:"1px solid rgba(255,255,255,0.08)" }}>
          ...
          <span style={{ fontSize:10, color:tk.onSurfaceVariant, letterSpacing:"0.04em" }}>{isFlag ? location.country : "Wikimedia Commons"}</span>
        </div>
      )}
      <div style={s.heroContent}>
        ...
        <div style={s.heroCard}>
          ...
          <div style={s.heroHiLo}>
            <span style={{ display:"flex", alignItems:"center", gap:4 }}><Icon name="arrow_upward" size={13} style={{ color:tk.primary }}/>H: {T(daily[0]?.high ?? current.temp)}°</span>
            <span style={{ display:"flex", alignItems:"center", gap:4 }}><Icon name="arrow_downward" size={13} style={{ color:tk.secondary }}/>B: {T(daily[0]?.low ?? current.temp)}°</span>
          </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius:"50%",
        backgroundColor: active ? `${tk.primary}20` : "rgba(255,255,255,0.05)",
        border: active ? `1px solid ${tk.primary}44` : "1px solid rgba(255,255,255,0.08)",
        color: active ? tk.primary : tk.onSurfaceVariant,
    ...
    <div style={s.app}>
        ...
        <nav style={s.nav}>
          {NAV_ITEMS.map(item => (
            <a key={item.id} href="#" onClick={e=>{ e.preventDefault(); onNavigate(item.id); }}
               className={item.id!==currentPage?"wni":""} style={item.id===currentPage ? s.navActive : s.navItem}>
      ...
      <main style={s.main}>
        <header style={s.header}>
          <SearchBar/>
          {/* Bouton aide — remplace la pastille PF et la cloche */}
          <HelpButton active={helpOpen} onClick={() => setHelpOpen(h => !h)}/>
        </header>
        ...
        <div style={s.content}>
          ...
          {!loading && !error && current && (<>
            ...
            {/* Grille metrics + graphique */}
            <div style={s.bottomGrid}>
              <div style={s.metricsGrid}>
                <div className="wmc" style={{ ...s.metricCard, transition:"all 0.18s" }}>
                  <div style={s.metricTopRow}><Icon name="humidity_mid" size={22} style={{ color:tk.primary }}/><span style={s.metricLabel}>Humidité</span></div>
                ...
                <div className="wmc" style={{ ...s.metricCard, transition:"all 0.18s" }}>
                  <div style={s.metricTopRow}><Icon name="sunny" size={22} style={{ color:tk.tertiary }}/><span style={s.metricLabel}>Indice UV</span></div>
                ...
                <div className="wmc" style={{ ...s.metricCard, transition:"all 0.18s" }}>
                  <div style={s.metricTopRow}><Icon name="air" size={22} style={{ color:tk.secondaryDim }}/><span style={s.metricLabel}>Vent</span></div>
                  <div><span style={s.metricValueLg}>{current.windSpeed}</span><span style={{ fontSize:16, fontWeight:400, color:tk.onSurfaceVariant }}> km/h</span></div>
                ...
                <div className="wmc" style={{ ...s.metricCard, transition:"all 0.18s" }}>
                  <div style={s.metricTopRow}><Icon name="compress" size={22} style={{ color:tk.error }}/><span style={s.metricLabel}>Pression</span></div>
                  <div><span style={s.metricValueMd}>{current.pressure}</span><span style={{ fontSize:14, fontWeight:400, color:tk.onSurfaceVariant }}> hPa</span></div>
                ...
                <div style={s.weeklyGrid}>
                  {daily.map((d,i) => (
                    <div key={i} className="wwc" style={{ ...s.weeklyCard, transition:"all 0.18s" }} onClick={() => setSelectedDay(d)}>
                      ...
                      {d.precipProb > 20 && <div style={{ display:"flex", alignItems:"center", gap:3, fontSize:10, color:tk.secondary }}><Icon name="water_drop" size={11} style={{ color:tk.secondary }}/>{d.precipProb}%</div>}
                      {d.sunrise && d.sunset && (
      ...
      {/* Modals via portal */}
      {selectedDay && createPortal(<DayDetailModal day={selectedDay} onClose={closeModal}/>, document.body)}
      {helpOpen    && createPortal(<HelpPanel page="dashboard" onClose={() => setHelpOpen(false)}/>, document.body)}
    </div>
  );
}
